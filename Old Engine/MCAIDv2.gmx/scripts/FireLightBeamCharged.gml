...
if (global.charge >= 60) then
{
   ...
   ab = instance_create(x+c1,y+d1,objLightCh);
   ab.direction = image_angle;
   ab.image_angle = ab.direction;
    
   cd = instance_create(x+c2,y+d2,objBeamShot);
   cd.sprite_index = sprLightShot;
   cd.image_speed = .5;
   cd.image_xscale = 1.5;
   cd.image_yscale = 1.5;
   ...
   for (i=0; i<360; i+=60)
   {
       ...
       {
           sprite_index = sprLightChargeFlak;
           direction = i+j;
           speed = 2;
           trails = 4;
           turnspeed = 0;
...
{
   ab = instance_create(x+c1,y+d1,objLight);
   ab.direction = random(6)+image_angle-3;
   ab.image_angle = ab.direction;
